...
pub fn run(config: &Config) -> anyhow::Result<()> {
    ...
    for repo in &sorted_repos {
        ...
        let commits_30d =
            db.tx(|conn| commit_store::count_commits_since(conn, repo.id, &thirty_days_ago))?;

        let last_pushed: Option<DateTime<Utc>> =
            repo.pushed_at.as_deref().and_then(|s| s.parse().ok());
    ...
    if let Some(featured) = read_featured_section(&index_path)? {
...
fn read_featured_section(index_path: &std::path::Path) -> anyhow::Result<Option<String>> {
    let Some(parent) = index_path.parent() else {
        return Ok(None);
    };
    let featured_path = parent.join("FEATURED.md");
    match fs::read_to_string(&featured_path) {
        Ok(contents) => {
...
mod tests {
    ...
    #[test]
    fn featured_section_is_trimmed_and_optional() {
        ...
        assert_eq!(read_featured_section(&index).unwrap(), None);

        fs::write(root.join("FEATURED.md"), "\n## Featured\n\nA project.\n").unwrap();
        assert_eq!(
            read_featured_section(&index).unwrap().as_deref(),
            Some("## Featured\n\nA project.")
        );
